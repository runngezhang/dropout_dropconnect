...
datasetDictionary = {'MNIST' : [28*28,[28,28],1,10] ,'SVHN':[32*32*3,[32,32],3,10] }
...
def test_lenet(learning_rate,momentum, n_epochs, nkerns, filter_shapes, pool_sizes,ignore_border,batch_size, verbose,smaller_set, activation,p,dataset,hidden_layer_type,crop,rotate ):
    """
    Wrapper function for testing convolutional network with 2 layers and 1 mlp layer where the hidden_layer_type can be specified.
        
        learning_rate - learning rate. Eg - 0.1
        n_epochs - number of epochs
        nkerns - [outputlayer0 kernels,outputlayer1 kernels,HiddenLayer neurons]
        filter_shapes - list of filter_shapes for each layer. Eg. [[2,2],[2,2]]
        poolsize - list of numbers of poolsize for each layer. Eg. [[1,1,],[1,1]]
        ignore_border - specifies if border is to be ignored for the pooling layers. Eg. [False,False]
        ...
        dataset - either 'MNIST' or 'SVHN'. type - string. 
        crop - boolean. 
        rotate - boolean. 

    """
    if dataset == 'MNIST' and crop == True:
        img_size = [24,24]
    else:
    ...
    ''' NEED TO FIGURE OUT HOW MANY CONV LAYERS ARE NEEDED AND THEIR CONFIGS'''
    ...
    print 'layer 2 begin : ',numpy.prod(end_size2)*nkerns[1]

    # construct a fully-connected sigmoidal layer
    layer2 = hidden_layer_type(
        rng=rng,
        input=layer2_input,
        n_in=numpy.prod(end_size2)*nkerns[1],
        n_out=nkerns[2],
    ...
    print 'layer 2 end : ',nkerns[2]

    consider_constant = [] 
    if hasattr(layer2, 'consider_constant'):
        print 'Found.'
        consider_constant.extend(layer2.consider_constant)
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            #print classifier.params[0].get_value()[0][:9]
            # iteration number
...
# test_lenet(learning_rate= 0.1, n_epochs=200, nkerns=[2,5,150], filter_shapes=[[2,2],[2,2]], pool_sizes=[[1,1,],[1,1]],ignore_border=[False,False],batch_size = 20, verbose = True,smaller_set= True, activation =T.tanh,p= 0.5,dataset= 'MNIST',hidden_layer_type = DropconnectHiddenLayer,momentum = 0.5,crop = False,rotate = False)
